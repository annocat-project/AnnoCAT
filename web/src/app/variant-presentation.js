...
function groupedEvidenceSection(title,groups,{open=false,className='',extra='',sectionKey=''}={}){
  ...
  return`<section class="detail-section annotation-section evidence-domain-section ${escapeHtml(className)}"><details class="evidence-domain collapsible-detail fui-accordion"${stateAttribute} ${expanded?'open':''}><summary><strong>${escapeHtml(title)}</strong>${detailToggleControl()}</summary><div class="key-evidence-subgroups">${groups.map(([groupTitle,items,empty,options])=>detailEvidenceSubgroup(groupTitle,items,empty,options)).join('')}${extra}</div></details></section>`
}
