...
export function isSpliceAiGeneField(field) {
  const source=String(field?.sourceId||'').toLowerCase(),leaf=String(field?.fieldPath||'').split(/[.\[\]]/).filter(Boolean).pop()?.replace(/[^a-z0-9]/gi,'').toLowerCase()||'';
  return(source==='spliceai'||source.startsWith('spliceai-')||source.startsWith('spliceai@'))&&leaf==='gene'
}
...
export function evidenceColumnPolicy(field) {
  ...
  if(isSpliceAiGeneField(field))return{selectable:false,recommended:false};
  const phenotypeSource=source==='hpo'||source.startsWith('hpo-')||source.startsWith('hpo@')||source==='gene-profile'||source.startsWith('gene-profile-')||source.startsWith('gene-profile@');
  if(phenotypeSource&&[
    'phenotyperelevance','genematch','matchedselecteditems','matcheditemtypes','genematchdetails',
    'profilelinked','includedgene','observedfeaturelinked','bestmatchingcondition',
    'directfeaturematches','absentfeatureconflict','selectedconditionmatches',
    'matchedselectedconditions','selectedconditionrelation','phenotypeevidencedetails'
  ].includes(leaf))return{selectable:false,recommended:false};
  ...
  return{selectable:field?.selectable!==false,recommended:field?.recommended===true}
}

export function referenceSnpId(evidence=[],fallback=''){
  const values=evidence.filter(item=>{const source=String(item?.sourceId||'').toLowerCase(),leaf=String(item?.fieldPath||'').split(/[.\[\]]/).filter(Boolean).pop()?.toLowerCase();return(source==='dbsnp'||source.startsWith('dbsnp-')||source.startsWith('dbsnp@'))&&leaf==='id'||leaf==='rsid'}).map(item=>item.value);
  for(const value of [...values,fallback]){const match=String(value??'').match(/\brs\d+\b/i);if(match)return match[0]}
  return''
}
...
function predictionSummaryFamily(item,presentation){
  const category=categoricalPredictionDefinition(item);
  if(category&&!category.excludeFromSummary&&String(category.fieldName).toLowerCase()!=='aloft_pred')return{
    key:String(category.fieldName).toLowerCase().replace(/_pred$/,''),
    label:readableTerm(String(category.fieldName).replace(/_pred$/i,''))
  };
  const predictor=calibratedPredictorDefinition(item);
  const summaryPredictors={revel:'REVEL','cadd-phred':'CADD PHRED'};
  return summaryPredictors[predictor?.id]&&(presentation.evidenceBand||presentation.nativeInterpretation)?{key:predictor.id,label:summaryPredictors[predictor.id]}:null
}
function predictionSummaryBar(items){
  ...
  items.forEach(item=>{
    const presentation=evidenceValuePresentation(item),family=predictionSummaryFamily(item,presentation);
    if(family&&(presentation.categoryInterpretation||presentation.evidenceBand||presentation.nativeInterpretation)&&!families.has(family.key))families.set(family.key,{...family,presentation,item});
    const band=presentation.evidenceBand;
    if(band?.role==='primary'&&band.predictorId!=='revel'&&!primaryCalibrated.has(band.evidenceGroup))primaryCalibrated.set(band.evidenceGroup,{presentation,item,label:band.predictorLabel})
  });
...
function renderVariantDetail(row,detail){
  ...
  const selectedTerms=consequenceValue(selected,'consequence_terms','Consequence'),selectedConsequenceId=selected._annocatConsequenceId,transcriptMetadataFields=new Set(['APPRIS','GENCODE_basic','TSL','Uniprot_acc','aaref','aaalt','aapos']),alignedEvidence=evidenceForSelectedConsequence(evidence,selectedConsequenceId,transcriptId,isTranscript),metadata=isTranscript?alignedEvidence.filter(item=>String(item.sourceId).toLowerCase()==='dbnsfp'&&transcriptMetadataFields.has(item.fieldPath)):[],combined=alignedEvidence.filter(item=>!dbnsfpTranscriptMetadata.has(item.fieldPath)&&!isSpliceAiGeneField(item)).map(item=>({...item,consequenceTerms:selectedTerms})),frequency=primaryAlleleFrequency(combined),frequencySourceItems=frequency?combined.filter(item=>String(item.sourceId||'').toLowerCase()===String(frequency.sourceId||'').toLowerCase()):combined,groupMaximum=groupMaximumAlleleFrequency(frequencySourceItems),frequencyDisplay=frequency?evidenceValuePresentation(frequency).display:reportHasFrequencyFields()?'Not reported':'Not available',groupMaximumDisplay=groupMaximum?evidenceValuePresentation(groupMaximum).display:reportHasFrequencyFields()?'Not reported':'Not available',domainGroups=combined.reduce((groups,item)=>{const domain=evidenceDomain(item);(groups[domain]??=[]).push(item);return groups},{}),sampleFacts=sampleOverview(variant);
