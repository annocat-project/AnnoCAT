...
export function createResultFilters({
  ...
  function loadFilterPreset(){syncState();const selected=$('#saved-filter-presets').value;if(selected==='')return;const index=Number(selected),preset=savedFilterPresets()[index];if(!preset)return;let unavailable=0;const selectable=new Set(selectableEvidenceEntries().map(entry=>entry.index)),rules=preset.rules.map(rule=>{if(rule.column!=='evidence')return rule;const fieldIndex=resultFieldCatalog.findIndex(field=>field.scope===rule.field?.scope&&field.sourceId===rule.field?.sourceId&&field.fieldPath===rule.field?.fieldPath);if(fieldIndex<0||!selectable.has(fieldIndex)){unavailable++;return null}return{...rule,column:`evidence:${fieldIndex}`}}).filter(Boolean);renderFilterRules(rules);if(selectionMode==='filtered')clearVariantSelection(true);$('#filter-message').textContent=unavailable?`${unavailable} saved annotation field${unavailable===1?' is':'s are'} not available in this result`:`Loaded “${preset.name}”`}
