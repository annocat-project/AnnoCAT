...
async function deleteResult(run){if(!run||!await confirmDestructiveAction({title:`Delete ${run.name}?`,message:'This result, its candidates, notes, phenotype data, and online annotations will be permanently deleted. Installed annotation data is not affected.',confirmLabel:'Delete result',cancelLabel:'Keep result'}))return;const response=await fetch(`/api/runs/${encodeURIComponent(run.id)}/delete`,{method:'POST',headers:{'X-AnnoCat-CSRF':'1'}}),body=await response.json();if(!response.ok){$('#result-library-status').textContent=body.error||'AnnoCAT did not delete the result';await refreshCompletedRuns();return}completedRuns=completedRuns.filter(item=>item.id!==run.id);resultPageMemory.clear();resultViewMemory.clear();if(currentResultRun?.id===run.id){currentResultRun=null;variants=[];closeVariantDetail()}renderCompletedRuns(completedRuns);await refreshAppStatus()}
