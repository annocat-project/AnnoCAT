...
import { compareNaturalResultOrder, createNaturalResultOrder } from './app/result-ordering.js';
import { MAX_VISIBLE_EVIDENCE_COLUMNS, shouldSelectColumnGroup } from './app/column-selection.js';
...
const columnGroups=[
  ...
  {label:'Sample call',columns:[['zygosity','Zygosity',true]]},
...
const coreColumnDetails={
  ...
  zygosity:['Zygosity','Relationship of the displayed alternate allele to the genotype. Results with multiple samples show the number of sample calls.'],
  gene:['Gene','Gene symbol selected for the representative transcript, or its stable Ensembl gene or transcript identifier when no symbol is assigned.'],
...
const resultSortModifierKeys=new Set();
document.addEventListener('keydown',event=>{if(['Control','Meta','Shift'].includes(event.key))resultSortModifierKeys.add(event.key)});
...
function queryEvidenceIndexes(indexes=[...visibleEvidence]){return[...new Set(indexes.flatMap(index=>[index,...evidenceDependencyIndexes(index)]))].sort((a,b)=>a-b)}
...
function recallResultCount(key){const total=resultCountMemory.get(key);if(!Number.isFinite(total))return null;resultCountMemory.delete(key);resultCountMemory.set(key,total);return total}
function rememberResultCount(key,total){if(!Number.isFinite(total))return;resultCountMemory.delete(key);resultCountMemory.set(key,total);while(resultCountMemory.size>RESULT_COUNT_MEMORY_LIMIT)resultCountMemory.delete(resultCountMemory.keys().next().value)}
function clearResultDataMemory(){resultPageMemory.clear();resultCountMemory.clear()}
...
function enableDetailResize(){
  ...
  const storageKey='annocat.detailWidth.v4',layout=$('#results .results-layout'),detail=$('#variant-detail'),tableWrap=$('#results .table-wrap'),toolbar=$('#results .toolbar'),tabs=$('#results .result-view-tabs');if(!layout||!detail)return;
  sessionStorage.removeItem('annocat.detailWidth');
  sessionStorage.removeItem('annocat.detailWidth.v2');
  sessionStorage.removeItem('annocat.detailWidth.v3');
  ...
  const handle=document.createElement('span');handle.className='variant-detail-resizer';handle.setAttribute('role','separator');handle.setAttribute('aria-label','Resize variant details');handle.setAttribute('aria-orientation','vertical');handle.tabIndex=0;handle.title='Drag to resize. Double-click to reset.';detail.prepend(handle);
...
async function loadAboutMetadata(){
  try{
    ...
    if(!response.ok)throw new Error(body.error||'Version information is not available');
...
function updateSetupModal(){const modal=$('#first-run'),wasHidden=modal.classList.contains('hidden'),hidden=lastSetupReady||setupDismissed;modal.classList.toggle('hidden',hidden);modal.classList.toggle('visible',!hidden);if(wasHidden&&!hidden)requestAnimationFrame(()=>$('#first-run .setup-modal').focus({preventScroll:true}))}
...
function taskActivityLabel(task){if(task.state!=='running')return{queued:'Queued',validating:'Verifying',pausing:'Pausing',cancelling:'Canceling',interrupted:'Needs attention · Resume available',failed:'Needs attention',paused:'Paused',cancelled:'Paused',downloaded:'Ready to install',ready:'Installed',completed:'Completed'}[task.state]||task.phase||task.state;if(task.phase==='streaming-to-fastvep')return task.kind==='annotation'?'Annotating':'Preparing cache';return{'recovery-scan':'Checking recovery data','recovery-input':'Preparing recovery data','recovery-merge':'Combining recovered data','indexing-variants':'Preparing result','indexing-evidence':'Preparing result','reconnecting':'Reconnecting','retrying':'Reconnecting','replaying':'Restoring data','building-cache':'Preparing cache','downloading-source-part':'Downloading','downloading':'Downloading','finalizing-annotation':'Preparing result','validating':'Verifying','verifying':'Verifying','reading-index':'Reading index','reading-indexes':'Reading index','publishing':'Saving result'}[task.phase]||(task.kind==='installation'?'Installing':task.kind==='download'?'Downloading':'Annotating')}
function taskJobView(task){
  const active=['queued','running','validating','pausing','cancelling','downloaded'].includes(task.state);
...
function confirmDestructiveAction({title,message,confirmLabel,cancelLabel='Keep data',danger=true}){
  ...
  confirmButton.classList.toggle('fui-button--danger',danger);
  confirmButton.classList.toggle('fui-button--primary',!danger);
  dialog.returnValue='';
...
function taskSectionHtml(kind,title,jobs){
  ...
  const description=kind==='failed'?'<p class="fui-caption">Resume recoverable work or cancel it to delete partial data.</p>':'';
...
function taskControlsAreActive(){return Boolean($('#jobs-list')?.querySelector('.fui-button:hover, .fui-button:focus'))}
function renderJobs(force=false){
  if(!force&&taskControlsAreActive()){taskRenderPending=true;return}
...
function flushPendingTaskRender(){if(taskRenderPending&&!taskControlsAreActive())renderJobs()}
$('#jobs-list').addEventListener('pointerout',event=>{
  const button=event.target.closest('.fui-button');
  if(button&&!button.contains(event.relatedTarget))requestAnimationFrame(flushPendingTaskRender)
});
$('#jobs-list').addEventListener('focusout',()=>requestAnimationFrame(flushPendingTaskRender));
function renderCompletedRuns(runs){
  const host=$('#completed-runs'),query=completedRunSearch.trim().toLowerCase(),filtered=query?runs.filter(run=>[run.name,run.originalName,run.assembly].some(value=>String(value||'').toLowerCase().includes(query))):runs;
  $('#result-library-status').textContent=query?`${filtered.length.toLocaleString()} of ${runs.length.toLocaleString()} results`:runs.length?`${runs.length.toLocaleString()} results`:'';
  host.innerHTML=filtered.length?filtered.map(run=>{const size=formatResultBytes(run.canonicalResultBytes);return`<article class="completed-run fui-card fui-card--row fui-card--menu-host" data-result-id="${escapeHtml(run.id)}"><div class="completed-run__summary"><strong>${escapeHtml(run.name)}</strong><small>${escapeHtml(formatDateTime(run.completedAt))} · ${escapeHtml(run.assembly)} · ${Number(run.variantCount).toLocaleString()} variants${size?` · ${size}`:''}</small></div><div class="completed-run__actions"><button type="button" class="fui-button fui-button--primary" data-result-open><span>Open</span><svg class="ui-icon" aria-hidden="true"><use href="#icon-arrow-right"/></svg></button><details class="fui-menu completed-run__menu" data-result-menu><summary class="fui-button fui-button--icon" aria-label="More actions" title="More actions"><svg class="ui-icon" aria-hidden="true"><use href="#icon-more-horizontal"/></svg></summary><div class="fui-popover fui-popover--menu fui-menu__popover" role="menu"><button type="button" class="fui-menu-item" data-result-rename role="menuitem"><svg class="ui-icon" aria-hidden="true"><use href="#icon-pencil"/></svg><span>Rename</span></button><button type="button" class="fui-menu-item fui-menu-item--danger" data-result-delete role="menuitem"><svg class="ui-icon" aria-hidden="true"><use href="#icon-trash-2"/></svg><span>Delete</span></button></div></details></div></article>`}).join(''):query?'<div class="empty-card compact fui-card"><svg class="ui-icon empty-icon"><use href="#icon-search"/></svg><h2>No matching results</h2><p>Change the search text and try again.</p></div>':'<div class="empty-card compact fui-card"><svg class="ui-icon empty-icon"><use href="#icon-folder"/></svg><h2>No results</h2><p>Completed results appear here.</p></div>';
  host.querySelectorAll('[data-result-id]').forEach(card=>{const run=runs.find(item=>item.id===card.dataset.resultId),menu=card.querySelector('[data-result-menu]');card.querySelector('[data-result-open]').addEventListener('click',()=>openCompletedRun(run));card.querySelector('[data-result-rename]').addEventListener('click',()=>{menu.removeAttribute('open');renameResult(run)});card.querySelector('[data-result-delete]').addEventListener('click',()=>{menu.removeAttribute('open');deleteResult(run)})})
}
function renderResultViewTabs(){const readOnly=Boolean(currentResultRun?.readOnly);if(readOnly)resultView='all';const candidates=resultView==='candidates';$('#all-variants-tab').classList.toggle('active',!candidates);$('#all-variants-tab').setAttribute('aria-selected',String(!candidates));$('#candidates-tab').classList.toggle('hidden',readOnly);$('#candidates-tab').classList.toggle('active',candidates);$('#candidates-tab').setAttribute('aria-selected',String(candidates));$('#candidate-count').textContent=candidateAlleles.size.toLocaleString()}
...
async function setCandidateMembership(alleleIds,add){if(!currentResultRun||!alleleIds.length)return;if(currentResultRun.readOnly)throw new Error('Candidates are not available in the demo');let body=null;for(let offset=0;offset<alleleIds.length;offset+=1000){const batch=alleleIds.slice(offset,offset+1000),response=await fetch(`/api/runs/${encodeURIComponent(currentResultRun.id)}/candidates`,{method:'POST',headers:{'Content-Type':'application/json','X-AnnoCat-CSRF':'1'},body:JSON.stringify({action:add?'add':'remove',alleleIds:batch})});body=await response.json();if(!response.ok)throw new Error(body.error||'Candidates could not be updated')}clearResultDataMemory();resultViewMemory.clear();resultCountSignature='';resultNaturalOrderSignature='';resultNaturalOrder.clear();candidateAlleles=new Set((body?.candidates||[]).map(candidate=>candidate.alleleId));renderResultViewTabs();if(resultView==='candidates'){variants=[];await openCompletedRun(currentResultRun,0)}else renderTable()}
...
async function openCompletedRun(run,offset=0){
  ...
  if(switchingRun){
    ...
    renderColumns();
    resultSorts=[];
    const savedPhenotypeProfile=await phenotypeProfilePromise;
    if(savedPhenotypeProfile)configureGeneQuery(savedPhenotypeProfile);else renderFilterRules();
    closeVariantDetail();
    document.querySelector('#results .results-heading h1').textContent=run.name;
    document.querySelector('#results .results-heading p').textContent='Loading the AnnoCAT result…';
    variants=[];
    resultTotal=0;
    renderTable();
  ...
  updateFilterButton(filterParameters.filterRules.length+filterParameters.evidenceFilters.length);
  ...
  try{
    const rememberedTotal=recallResultCount(countSignature),knownTotal=appending||countSignature===resultCountSignature?resultTotal:Number.isFinite(rememberedTotal)?rememberedTotal:!filtered?(resultView==='all'?Number(run.variantCount)||0:candidateAlleles.size):null;
    ...
    const incoming=(body.rows||[]).map(row=>({...row,gene:row.geneSymbol||row.geneId||row.transcriptId||'',clinvar:'',inheritance:'',score:''}));
    if(appending){
    ...
    const returnedTotal=body.total===null||body.total===undefined?null:Number(body.total);
    resultTotal=Number.isFinite(returnedTotal)?returnedTotal:Number.isFinite(knownTotal)?knownTotal:null;
    ...
    if(Number.isFinite(resultTotal))rememberResultCount(countSignature,resultTotal);
    resultHasMore=Boolean(body.hasMore);
    if(!resultHasMore)rememberLoadedNaturalOrder(countSignature);
    renderTable();
...
async function refreshCurrentResultSchema({sourceId='',preferredFields=[],configureCatalog=null}={}){
  ...
  variants=[];
  resultLoading=false;
  resultOperation='Loading annotation fields…';
  closeVariantDetail();
...
async function refreshCompletedRuns(){const response=await fetch('/api/runs'),body=await response.json();if(!response.ok)throw new Error(body.error||'Results are not available');completedRuns=body.runs||[];renderCompletedRuns(completedRuns);renderJobs()}
...
async function shareCurrentReport(){if(!currentResultRun)return;const includesGeneProfile=phenotypeFeature.hasActiveProfile();if(includesGeneProfile&&!await confirmDestructiveAction({title:'Export gene selections?',message:'The export will include selected features, conditions, pathways, entered genes, and derived gene matches. Candidates are also included. Notes stay on this computer.',confirmLabel:'Export result',cancelLabel:'Cancel',danger:false}))return;const button=$('#share-report'),description=document.querySelector('#results .results-heading p'),previous=button.textContent;button.disabled=true;button.textContent='Exporting…';try{const response=await fetch(`/api/runs/${encodeURIComponent(currentResultRun.id)}/share`,{method:'POST',headers:{'X-AnnoCat-CSRF':'1'}}),body=await response.json();if(!response.ok)throw new Error(body.error||'AnnoCAT did not export the result');if(body.path)description.textContent=`Exported AnnoCAT result · ${formatResultBytes(body.bytes)} · ${body.path} Candidates${includesGeneProfile?' and gene selections are':' are'} included. Notes stay on this computer.`;}catch(error){description.textContent=`AnnoCAT did not export the result: ${error.message}`}finally{button.disabled=false;button.textContent=previous}}
async function renameResult(run){if(!run)return;const name=prompt('Result name',run.name);if(name===null||name.trim()===run.name)return;const response=await fetch(`/api/runs/${encodeURIComponent(run.id)}/name`,{method:'POST',headers:{'Content-Type':'application/json','X-AnnoCat-CSRF':'1'},body:JSON.stringify({name:name.trim()})}),body=await response.json();if(!response.ok){$('#result-library-status').textContent=body.error||'AnnoCAT did not rename the result';return}run.name=body.name;if(currentResultRun?.id===run.id){currentResultRun.name=body.name;document.querySelector('#results .results-heading h1').textContent=body.name}renderCompletedRuns(completedRuns)}
async function renameCurrentResult(){return renameResult(currentResultRun)}
async function deleteResult(run){if(!run||!await confirmDestructiveAction({title:`Delete ${run.name}?`,message:'This result, its candidates, notes, gene selections, and online annotations will be permanently deleted. Installed annotation data is not affected.',confirmLabel:'Delete result',cancelLabel:'Keep result'}))return;const response=await fetch(`/api/runs/${encodeURIComponent(run.id)}/delete`,{method:'POST',headers:{'X-AnnoCat-CSRF':'1'}}),body=await response.json();if(!response.ok){$('#result-library-status').textContent=body.error||'AnnoCAT did not delete the result';await refreshCompletedRuns();return}completedRuns=completedRuns.filter(item=>item.id!==run.id);clearResultDataMemory();resultViewMemory.clear();if(currentResultRun?.id===run.id){currentResultRun=null;variants=[];closeVariantDetail()}renderCompletedRuns(completedRuns);await refreshAppStatus()}
...
const phenotypeFeature=createPhenotypeFeature({$,escapeHtml,prototypeIcon,showPage,onApply:phenotypeProfileApplied});
async function openPhenotypeDialog(){const popover=$('#phenotype-popover'),wasOpen=Boolean(popover&&!popover.classList.contains('hidden'));closeResultPopovers();if(!wasOpen)return phenotypeFeature.open(currentResultRun,resourceStates)}
...
function displayedSearchEvidenceColumns(search=$('#search').value.trim()){return search?queryEvidenceIndexes():[]}
...
function updateFilterButton(count){const button=$('#filters'),badge=button?.querySelector('[data-filter-count]');if(!badge)return;badge.textContent=count.toLocaleString();badge.classList.toggle('hidden',count===0);button.setAttribute('aria-label',count?`Filters: ${count.toLocaleString()} applied`:'Filters')}
function resultPageParameters({offset=0,limit=200,knownTotal=null,exactTotal=false,generation=null,querySession=resultQuerySession,includeSort=true}={}){
  const search=$('#search').value.trim(),filters=resultFilterParameters(),evidenceColumns=queryEvidenceIndexes(),parameters=new URLSearchParams({offset:String(offset),limit:String(limit),search});
...
function updateResultHeading(run=currentResultRun,filtered=hasActiveResultQuery()){
  ...
  document.querySelector('#results .results-heading p').textContent=`${run.assembly} · ${count} ${filtered?'matching ':''}${resultView==='candidates'?'candidates':'variants'}`
}
async function ensureExactResultTotal({generation=resultRequestGeneration}={}){
  ...
  const promise=(async()=>{
    try{
      ...
      rememberResultCount(countSignature,total);
...
async function exportFilteredSelection(format){if(!currentResultRun)return;const description=document.querySelector('#results .results-heading p'),shown=selectedRowExportColumns(),columnsToExport=shown.map(([key])=>key==='__rsid'?'rsid':key),columnLabels=shown.map(([,label])=>label),parameters=resultFilterParameters(),search=$('#search').value.trim(),filters={search,evidenceColumns:displayedSearchEvidenceColumns(search),filterRules:parameters.filterRules,evidenceFilters:parameters.evidenceFilters,excludedAlleleIds:[...excludedFilteredAlleles]},candidateOnly=resultView==='candidates';description.textContent=format==='genesTxt'?'Choose where to save genes from matching variants…':'Choose where to save matching variants…';try{const response=await fetch(`/api/runs/${encodeURIComponent(currentResultRun.id)}/export`,{method:'POST',headers:{'Content-Type':'application/json','X-AnnoCat-CSRF':'1'},body:JSON.stringify({format,filters,columns:columnsToExport,columnLabels,candidateOnly})}),body=await response.json();if(!response.ok)throw new Error(body.error||'Matching-variant export failed');if(!body.path){description.textContent='Export canceled.';return}description.textContent=format==='genesTxt'?`Exported ${Number(body.genes||0).toLocaleString()} unique genes from ${Number(body.rows||0).toLocaleString()} matching variants · ${body.path}`:`Exported ${Number(body.rows||0).toLocaleString()} matching variants · ${body.path}`}catch(error){description.textContent=`AnnoCAT did not export the matching variants: ${error.message}`}}
...
function rowReferenceSnpId(row){const evidence=Object.entries(row.evidence||{}).map(([index,value])=>({...resultFieldCatalog[Number(index)],value:decodeEvidenceValue(value)}));return referenceSnpId(evidence,row.variantId)}
function selectedRowExportColumns(){const shown=displayColumns();if(shown.some(([key])=>{if(!key.startsWith('evidence:'))return false;const field=resultFieldCatalog[Number(key.slice(9))]||{},leaf=String(field.fieldPath||'').split(/[.\[\]]/).filter(Boolean).pop()?.toLowerCase();return fieldSourceIs(field,'dbsnp')&&leaf==='id'||leaf==='rsid'}))return shown;const insertion=shown.reduce((position,[key],index)=>key==='alternate'||key==='variantId'?index+1:position,shown.length);shown.splice(insertion,0,['__rsid','rsID']);return shown}
async function exportSelectedRows(){if(selectionMode==='filtered')return exportFilteredSelection('rowsCsv');if(!selectedAlleles.size)return;const shown=selectedRowExportColumns(),rows=[shown.map(([,label])=>csvCell(label)).join(',')];selectedAlleles.forEach(id=>{const row=selectedVariantRows.get(id);if(row)rows.push(shown.map(([key])=>csvCell(key==='__rsid'?rowReferenceSnpId(row):resultColumnValue(row,key))).join(','))});if(rows.length===1)return;const description=document.querySelector('#results .results-heading p'),blob=new Blob([`\uFEFF${rows.join('\r\n')}\r\n`],{type:'text/csv;charset=utf-8'});try{if(!await saveExportBlob(blob,exportFilename('selected-visible-columns.csv'),'Comma-separated values','.csv'))return;description.textContent=`Exported ${rows.length-1} selected variants with ${shown.length} columns.`}catch(error){description.textContent=`Could not export selected rows: ${error.message}`}}
...
function profileEvidenceValue(row,index){
  const field=resultFieldCatalog[index]||{},value=resultColumnRawValue(row,`evidence:${index}`),score=evidenceValuePresentation({...field,consequenceTerms:row.consequence},value).display;
...
function resultColumnCellHtml(row,key){
  ...
  if(key==='gene'){const value=resultColumnValue(row,key),fallback=!row.geneSymbol&&value,title=fallback?`No gene symbol is assigned; showing the ${row.geneId?'Ensembl gene ID':'transcript ID'}.`:'';return`<span ${title?`title="${escapeHtml(title)}"`:''}>${escapeHtml(value)}</span>`}
  if(key.startsWith('evidence:')){
    ...
    if(profileEvidenceField(field)){
      const value=profileEvidenceValue(row,index);
      return`<span class="profile-evidence-cell" title="${escapeHtml(value.tooltip)}">${escapeHtml(value.display)}</span>`
    }
  }
  return escapeHtml(resultColumnValue(row,key))
}
function resultColumnTooltip(key,description,sourceId){
  ...
  if(field.fieldPath)parts.push(`Field: ${field.fieldPath}.`);
...
function renderTableBase(event){
  ...
  const readOnly=Boolean(currentResultRun?.readOnly);
  ['#share-report','#rename-report','#case-notes-button'].forEach(selector=>$(selector).classList.toggle('hidden',!currentResultRun||readOnly));
  ['#phenotypes','#favor'].forEach(selector=>$(selector).classList.toggle('hidden',readOnly));
  if(!currentResultRun)$('#case-notes-panel').classList.add('hidden');
  const shown=displayColumns(),allFiltered=selectionMode==='filtered',hasResults=Number.isFinite(resultTotal)?resultTotal>0:variants.length>0,allVisibleCandidates=hasResults&&variants.length>0&&variants.every(row=>candidateAlleles.has(row.alleleId)),candidateAction=allVisibleCandidates?'Remove all matching variants from candidates':'Add all matching variants to candidates',emptyMessage=resultLoading?'Loading variants…':hasActiveResultQuery()?'No variants match the current search or filters.':resultView==='candidates'?'No candidates have been added yet.':'This result contains no displayable variants.';
  $('#head').innerHTML=`<th class="selection-cell fui-data-grid__utility-cell"><input id="result-select-all-checkbox" class="fui-checkbox" type="checkbox" aria-label="Select all matching variants" title="Select matching variants"></th><th class="candidate-cell fui-data-grid__utility-cell"><button id="candidate-all" type="button" class="candidate-toggle candidate-column-heading fui-data-grid__icon-button ${allVisibleCandidates?'active':''}" aria-label="${readOnly?'Candidates are not available in the demo':candidateAction}" title="${readOnly?'Candidates are not available in the demo':candidateAction}" ${readOnly?'disabled':''}>${prototypeIcon('star')}<span class="legacy-icon">${allVisibleCandidates?'★':'☆'}</span></button></th>`+shown.map(([key,label,description,,sourceId])=>{const tooltip=resultColumnTooltip(key,description,sourceId),sortable=key.startsWith('evidence:')||!nonSortableCoreColumns.has(key);if(!sortable)return`<th title="${escapeHtml(tooltip)}"><span class="column-sort fui-data-grid__sort-button"><span>${escapeHtml(label)}</span></span></th>`;const priority=resultSorts.findIndex(sort=>sort.key===key),sort=priority<0?null:resultSorts[priority],indicator=sort?`${resultSorts.length>1?priority+1:''}${sort.direction==='asc'?'▲':'▼'}`:'↕',ariaSort=priority===0?(sort.direction==='asc'?'ascending':'descending'):'none';return`<th title="${escapeHtml(tooltip)}" aria-sort="${ariaSort}"><button type="button" class="column-sort fui-data-grid__sort-button" data-sort-column="${escapeHtml(key)}" aria-label="${escapeHtml(`${label}. ${tooltip} Click to sort. Ctrl- or Shift-click to add another sort column.`)}"><span>${escapeHtml(label)}</span><b aria-hidden="true">${indicator}</b></button></th>`}).join('');
  $('#rows').innerHTML=variants.length?variants.map(row=>{const candidate=candidateAlleles.has(row.alleleId),selected=allFiltered?!excludedFilteredAlleles.has(row.alleleId):selectedAlleles.has(row.alleleId),classes=[row.alleleId===selectedAlleleId?'selected-variant':'',selected?'selection-active':''].filter(Boolean).join(' '),candidateLabel=readOnly?'Candidates are not available in the demo':`${candidate?'Remove from':'Add to'} candidates`;return`<tr ${row.alleleId?`tabindex="0" data-allele-id="${escapeHtml(row.alleleId)}" aria-label="Open details for ${escapeHtml(`${row.chromosome}:${row.position} ${row.reference}>${row.alternate}`)}"`:''} class="${classes}"><td class="selection-cell fui-data-grid__utility-cell">${row.alleleId?`<input class="fui-checkbox" type="checkbox" data-select-allele="${escapeHtml(row.alleleId)}" aria-label="Select ${escapeHtml(`${row.chromosome}:${row.position} ${row.reference}>${row.alternate}`)}" ${selected?'checked':''}>`:''}</td><td class="candidate-cell fui-data-grid__utility-cell">${row.alleleId?`<button type="button" class="candidate-toggle fui-data-grid__icon-button ${candidate?'active':''}" data-toggle-candidate="${escapeHtml(row.alleleId)}" aria-label="${candidateLabel}" title="${candidateLabel}" ${readOnly?'disabled':''}>${prototypeIcon('star')}<span class="legacy-icon">${candidate?'★':'☆'}</span></button>`:''}</td>${shown.map(([key])=>`<td>${resultColumnCellHtml(row,key)}</td>`).join('')}</tr>`}).join(''):emptyMessage?`<tr class="empty-result-row"><td class="fui-data-grid__empty-cell" colspan="${Math.max(2,shown.length+2)}"><span class="fui-data-grid__empty-content">${escapeHtml(emptyMessage)}</span></td></tr>`:'';
  $('#head').querySelectorAll('[data-sort-column]').forEach(button=>button.addEventListener('click',event=>changeResultSort(button.dataset.sortColumn,event.shiftKey||event.ctrlKey||event.metaKey||resultSortModifierKeys.size>0)));
  const selectAll=$('#result-select-all-checkbox');selectAll.checked=allFiltered&&!excludedFilteredAlleles.size;selectAll.indeterminate=allFiltered?excludedFilteredAlleles.size>0:selectedAlleles.size>0;selectAll.disabled=!hasResults;selectAll.addEventListener('change',async()=>{if(!selectAll.checked){clearVariantSelection();return}selectAll.disabled=true;try{await selectAllFilteredVariants()}catch(error){document.querySelector('#results .results-heading p').textContent=`Could not select matching variants: ${error.message}`;renderTable()}});const candidateAll=$('#candidate-all');candidateAll.disabled=!hasResults||readOnly;candidateAll.addEventListener('click',toggleHeaderCandidates);
  updateSelectionControls()
}
...
function rememberLoadedNaturalOrder(signature){
  if(!variants.length||variants.length>500)return;
  resultNaturalOrderSignature=signature;
  resultNaturalOrder=createNaturalResultOrder(variants)
}
function sortFullyLoadedResults(){
  ...
  variants.sort((left,right)=>{
    ...
    return compareNaturalResultOrder(order,left,right)
  });
  renderTable()
}
async function openSortedResult(run){
  resultSortStatusController?.abort();
  const generation=++resultSortStatusGeneration,controller=new AbortController();
  resultSortStatusController=controller;
  try{
    const parameters=resultPageParameters({offset:0,limit:1}),response=await fetch(`/api/runs/${encodeURIComponent(run.id)}/query-cache-status?${parameters}`,{signal:controller.signal}),body=await response.json();
    if(!response.ok)throw new Error(body.error||'Sort status could not be checked');
    if(generation!==resultSortStatusGeneration||run!==currentResultRun)return;
    if(!body.ready){resultOperation='Preparing column for sorting…';updateResultPageStatus()}
  }catch(error){if(error.name==='AbortError')return}
  finally{
    if(generation===resultSortStatusGeneration&&run===currentResultRun){
      if(resultSortStatusController===controller)resultSortStatusController=null;
      resultHasMore=false;
      openCompletedRun(run,0)
    }
  }
}
function changeResultSort(key,additive=false){const index=resultSorts.findIndex(sort=>sort.key===key),existing=index<0?null:resultSorts[index];if(additive){if(!existing)resultSorts.push({key,direction:'asc'});else if(existing.direction==='asc')existing.direction='desc';else resultSorts.splice(index,1)}else if(index===0){resultSorts=existing.direction==='asc'?[{key,direction:'desc'}]:[]}else resultSorts=[{key,direction:'asc'}];$('#results .table-wrap').scrollTop=0;resultRequestController?.abort();resultCountRequest?.controller.abort();resultQueryError='';resultOperation='Sorting…';resultLoading=true;updateResultPageStatus();if(!currentResultRun){requestAnimationFrame(()=>{sortFullyLoadedResults();resultLoading=false;resultOperation='';updateResultPageStatus()});return}if(Number.isFinite(resultTotal)&&variants.length===resultTotal&&resultTotal<=500&&resultNaturalOrderSignature===currentResultCountSignature()){requestAnimationFrame(()=>{sortFullyLoadedResults();resultLoading=false;resultOperation='';updateResultPageStatus()});return}void openSortedResult(currentResultRun)}
...
async function openVariantDetail(alleleId){
  ...
  try{
    ...
    if(!response.ok)throw new Error(result.error||'Variant details are not available');
...
function renderColumns(){
  ...
  const search=`<div class="column-menu-searchbar"><input class="column-menu-search fui-input" type="search" data-column-search aria-label="Search visible columns" placeholder="Search columns, data sources, descriptions, or raw keys"></div>`;
  const preference=`<div class="column-menu-toolbar"><label class="column-name-preference"><input class="fui-checkbox" type="checkbox" data-human-readable-columns ${humanReadableColumnNames?'checked':''}><span><strong>Human-readable column names</strong><small>Turn off to show raw annotation field keys.</small></span></label><button type="button" class="fui-button" data-restore-default-columns>Restore recommended</button></div>`;
  ...
  menu.querySelectorAll('[data-evidence-index]').forEach(box=>box.addEventListener('change',event=>{
    ...
    if(event.target.checked&&visibleEvidence.size>=MAX_VISIBLE_EVIDENCE_COLUMNS){
...
function defaultWizardProfile(savedProfile){const hasSupplementarySource=sources.some(source=>source.fastvepSource&&source.id!=='fastvep'&&resourceStates[source.id]?.ready),preferred=hasSupplementarySource?savedProfile:'online';return profiles.some(profile=>profile.id===preferred)?preferred:profiles[0]?.id}
function applyDefaultWizardProfile(savedProfile=localStorage.getItem('annocat.defaultProfile')||'wgs'){const profileId=defaultWizardProfile(savedProfile);if(profileId)$('#profile').value=profileId;renderWizardSources();updateWizardReadiness()}
function profileInstallSourceIds(profile){return[...new Set([...(profile?.sourceIds||[]),...(profile?.knowledgeSourceIds||[])])]}
function renderProfiles(){const ordered=[...profiles].sort((a,b)=>a.id==='wgs'?-1:b.id==='wgs'?1:0),preferred=ordered.some(profile=>profile.id==='wgs')?'wgs':ordered[0]?.id;profiles=ordered;$('#profile').innerHTML=ordered.map(profile=>`<option value="${escapeHtml(profile.id)}" ${profile.id===preferred?'selected':''}>${escapeHtml(profile.name)}${profile.id==='wgs'?' (recommended)':''}</option>`).join('')+'<option value="custom">Custom</option>';const host=$('#profile-install-actions'),installProfiles=ordered.filter(profile=>profileInstallSourceIds(profile).length||(profile.serviceIds||[]).length===0);host.innerHTML=installProfiles.map(profile=>{const label=profile.name,names=['Core annotation data',...profileInstallSourceIds(profile).map(id=>sources.find(source=>source.id===id)?.name||id)];return`<article class="fui-card fui-card--content-compact"><strong class="fui-card__title">${escapeHtml(label)}${profile.id==='wgs'?' · Recommended':''}</strong><small class="fui-card__metadata">${escapeHtml(names.join(' · '))}</small><button type="button" class="fui-button" data-profile-install="${escapeHtml(profile.id)}">Install ${escapeHtml(label)}</button></article>`}).join('');host.querySelectorAll('[data-profile-install]').forEach(button=>button.addEventListener('click',()=>showProfileInstallReview(button.dataset.profileInstall)))}
function sourceLicenseNote(source){if(source.id==='hpo')return'<small class="source-license-note">Uses versioned HPO, MONDO, and HGNC releases. Review the <a href="https://human-phenotype-ontology.github.io/license.html" target="_blank" rel="noopener noreferrer">HPO terms</a>, <a href="https://mondo.monarchinitiative.org/pages/download/" target="_blank" rel="noopener noreferrer">MONDO CC BY 4.0 terms</a>, and <a href="https://www.genenames.org/about/license/" target="_blank" rel="noopener noreferrer">HGNC CC0 terms</a>.</small>';if(source.delivery==='managed-public-noncommercial')return'<small class="source-license-note">Non-commercial use; commercial use requires a CADD license.</small>';if(source.delivery==='user-supplied-licensed')return`<small class="source-license-note">Import files obtained under your ${escapeHtml(source.name)} license.</small>`;return''}
...
function updateWizardReadiness(){const host=$('#wizard-readiness');if(!host)return;const profile=selectedProfile(),selected=enabledSourceIds(),missing=(profile?.sourceIds||[]).filter(id=>!resourceStates[id]?.ready),usesFavor=(profile?.serviceIds||[]).includes('favor-variant-annotation');let tone='ready',title='',detail='';if(!lastSetupReady){tone='blocked';title='Annotation is not ready';detail='Install the GRCh38 reference and transcript cache before you start an annotation.'}else if(profile&&missing.length){tone='partial';title=`${selected.length} available profile source${selected.length===1?'':'s'} selected`;detail=`${missing.length} profile source${missing.length===1?' is':'s are'} not installed. Continue with the available sources or open Data sources.`}else if(selected.length){title=`${selected.length} data source${selected.length===1?'':'s'} selected`;detail='The selected data sources are installed and ready.'}else if(usesFavor){tone=favorOnline.isEnabled()?'ready':'partial';title=favorOnline.isEnabled()?'Core annotation with online annotations':'Core annotation; FAVOR is disabled';detail=favorOnline.isEnabled()?'Online annotations are available for selected or matching variants in the AnnoCAT result.':'Enable FAVOR in Data sources to use online annotations.'}else{tone='partial';title='Core annotation only';detail='No supplementary data sources are selected.'}const appearance={ready:'success',partial:'info',blocked:'danger'}[tone];host.className=`wizard-readiness fui-status-message fui-status-message--${appearance} ${tone}`;host.querySelector('strong').textContent=title;host.querySelector('p').textContent=detail;host.querySelector('button').classList.toggle('hidden',tone==='ready')}
function selectedVcfProblem(file){if(!file)return null;if(file.error)return`This file could not be read: ${file.error}`;if(file.hasRecords===false)return'This VCF contains no variant records.';if(file.assembly==='GRCh37')return'GRCh37, b37, and hg19 inputs are not supported in this release. Select a GRCh38 VCF.';if(file.assembly&&file.assembly!=='GRCh38')return`AnnoCAT supports GRCh38 inputs only; this file declares ${file.assembly}.`;return null}
...
function selectedVcfBuildUnknown(){return selectedVcfSummaries.length!==selectedPaths.length||selectedVcfSummaries.some(file=>!file.assembly)}
function confirmSelectedVcfBuild(){const count=Math.max(1,selectedVcfSummaries.filter(file=>!file.assembly).length),plural=count!==1;return confirmDestructiveAction({title:'Confirm genome build',message:`The VCF header${plural?'s do':' does'} not identify the genome build. Continue only if ${plural?'these files use':'this file uses'} GRCh38 coordinates and reference alleles. AnnoCAT will validate sequence alleles against its installed GRCh38 reference.`,confirmLabel:`Yes, ${plural?'these are':'this is'} GRCh38`,cancelLabel:'Go back',danger:false})}
...
function coreAnnotationSize(items=resourcePlan.resources.filter(item=>coreResourceIds.has(item.id)),formatter=formatDataSize){const downloadBytes=items.reduce((sum,item)=>sum+Number(item.downloadBytes||0),0),preparedBytes=items.reduce((sum,item)=>sum+Number(resourceStates[item.id]?.prepare?.preparedBytes||0),0),download=downloadBytes?`${formatter(downloadBytes)} download`:'Download size not available';return preparedBytes?`${download} · ${formatter(preparedBytes)} installed`:`${download} · installed size measured during installation`}
function renderReview(){const ids=enabledSourceIds(),keepVcf=$('#keep-annotated-vcf').checked;$('#review-summary').innerHTML=`<div class="fui-summary-grid__item"><span>Input</span><strong>${recoveryFiles?'Annotation recovery':`${selectedPaths.length} VCF${selectedPaths.length===1?'':'s'}`}</strong></div><div class="fui-summary-grid__item"><span>Annotation order</span><strong>${recoveryFiles?'Resume remaining records':selectedPaths.length===1?'One annotation':'One annotation at a time'}</strong></div><div class="fui-summary-grid__item"><span>Profile</span><strong>${escapeHtml($('#profile').selectedOptions[0].text)}</strong></div><div class="fui-summary-grid__item"><span>Output</span><strong>${escapeHtml($('#output-folder').value||'Not selected')}</strong></div><div class="fui-summary-grid__item"><span>Results</span><strong>AnnoCAT result${keepVcf?' + annotated VCF':''}</strong></div><div class="review-summary-empty fui-summary-grid__item" aria-hidden="true"></div>`;const required=`<div class="fui-key-value-row" data-resource-review="core"><span class="readiness-dot"></span><strong>Core annotation data</strong><em data-reference-state>Checking…</em></div>`;$('#resource-review').innerHTML=required+ids.map(id=>`<div class="fui-key-value-row" data-resource-review="${escapeHtml(id)}"><span class="readiness-dot"></span><strong>${escapeHtml(resourceTitle(id))}</strong><em data-resource-state="${escapeHtml(id)}">Checking…</em></div>`).join('');updateReviewResourceStates();refreshAppStatus().catch(console.error)}
...
function updateReviewReadiness(){const host=$('#review-readiness');if(!host)return;const ids=enabledSourceIds(),selectedReady=ids.every(id=>resourceStates[id]?.ready),ready=lastSetupReady&&selectedReady;host.className=`wizard-readiness fui-status-message fui-status-message--${ready?'success':'danger'} ${ready?'ready':'blocked'}`;host.querySelector('strong').textContent=ready?'Ready to annotate':'Annotation data is not ready';host.querySelector('p').textContent=ready?`${ids.length?`${ids.length} supplementary data source${ids.length===1?'':'s'} plus core annotation data`:'Core annotation data'} will be used.`:'Install the missing core or selected data sources before you start.';host.querySelector('button').classList.toggle('hidden',ready)}
async function refreshAnnotationStatus(snapshot){let body=snapshot;if(!body){const response=await fetch('/api/annotations/status');body=await response.json();if(!response.ok)throw new Error(body.error||'Annotation status is not available')}const previous=lastAnnotationState.state;lastAnnotationState=body;if(previous==='running'&&body.state==='completed'){await refreshCompletedRuns();showPage('browse')}return body}
async function refreshTasks(snapshot){let body=snapshot;if(!body){const response=await fetch('/api/tasks');body=await response.json();if(!response.ok)throw new Error(body.error||'Task status is not available')}lastTaskSnapshots=body.tasks||body||[];renderJobs();return lastTaskSnapshots}
...
async function handleAnnotationTaskAction(runId,action,button){
  if(action==='cancel'&&!await confirmDestructiveAction({title:'Cancel this annotation?',message:'The annotation will stop and its incomplete output will be discarded. Completed annotations and installed data sources are not affected.',confirmLabel:'Cancel annotation',cancelLabel:'Continue annotation'}))return;
  if(action==='discard'&&!await confirmDestructiveAction({title:'Cancel this interrupted annotation?',message:'The interrupted annotation, its checkpoint, and all partial output will be permanently deleted. Completed annotations and installed data sources are not affected.',confirmLabel:'Cancel annotation',cancelLabel:'Keep partial data'}))return;
  const original=button.textContent;
  button.disabled=true;
  button.textContent=action==='resume'?'Resuming…':action==='pause'?'Pausing…':action==='discard'?'Deleting…':'Canceling…';
  try{
    if(action==='resume'){
      ...
      if(!response.ok||!body.pauseRequested)throw new Error(body.error||'Annotation is not active')
...
async function refreshPaths(){
  ...
  document.querySelectorAll('#wizard-resource-path').forEach(element=>element.textContent=portablePaths.resourceDirectory||'Not available');
  $('#settings-resource-path').value=portablePaths.resourceDirectory||'Not available';
  $('#settings-downloads-path').value=portablePaths.downloads||'Not available';
  $('#settings-results-path').value=portablePaths.runs||'Not available';
...
const settingsStoragePickers={
  ...
  results:{route:'results',message:'AnnoCAT stores new and imported results here.'}
};
...
function resourceTaskProgress(task,total,completed){
  ...
  if(task.state==='running'&&Number(task.throughputBytesPerSecond)>0)parts.push(`${task.phase==='building-cache'?'Build speed ':''}${formatDataSize(task.throughputBytesPerSecond)}/s`);
  return parts.join(' · ')
}
...
function applyResourceStatus(id,{download,prepare}){const preparing=prepare.state==='running',prepareQueued=prepare.state==='queued',downloading=download.state==='running',validating=download.state==='validating',queued=download.state==='queued'||prepareQueued,pausing=download.state==='pausing'||prepare.state==='pausing'||prepare.cancelRequested,cancelling=download.state==='cancelling'||prepare.state==='cancelling',ready=prepare.state==='ready',rebuildRequired=prepare.state==='rebuild-required',hasArchive=download.state==='downloaded',failed=download.state==='failed'||prepare.state==='failed',hasPartial=download.downloadedBytes>0&&!hasArchive,hasPreparedPartial=Number(prepare.completedChromosomes||0)>0&&!ready,paused=['paused','cancelled'].includes(download.state)||['paused','cancelled'].includes(prepare.state)||hasPartial||(prepare.state==='idle'&&hasPreparedPartial),hasManagedData=ready||hasArchive||hasPartial||hasPreparedPartial||!['idle','missing'].includes(prepare.state),label=ready?'Installed':rebuildRequired?'Rebuild required':cancelling?'Canceling':pausing?'Pausing':preparing?'Installing':validating?'Verifying':failed?'Needs attention':hasArchive&&!paused?'Ready to install':downloading?'Downloading':queued?'Queued':paused?'Paused':'Not installed',busy=preparing||downloading||validating||queued||pausing||cancelling,result={download,prepare,ready,label};resourceStates[id]=result;document.querySelectorAll(`[data-resource-state="${id}"]${id==='dbnsfp'?', [data-dbnsfp-state]':''}`).forEach(node=>node.textContent=label);document.querySelectorAll(`[data-resource-review="${id}"]`).forEach(row=>row.classList.toggle('ready',ready));document.querySelectorAll(`[data-resource-storage="${id}"]`).forEach(node=>node.textContent=resourceSize(id,result));document.querySelectorAll(`[data-install="${id}"]`).forEach(button=>{const action=rebuildRequired?'Rebuild cache':paused?'Resume':'Install',icon=rebuildRequired?'refresh-cw':paused?'play':'download';button.disabled=busy;button.innerHTML=`${prototypeIcon(icon)}<span>${action}</span>`;button.classList.toggle('resume',paused||rebuildRequired);button.classList.toggle('install',!paused&&!rebuildRequired);button.classList.toggle('hidden',busy||ready)});document.querySelectorAll(`[data-update="${id}"]`).forEach(button=>button.classList.toggle('hidden',!ready||busy));document.querySelectorAll(`[data-source-overflow="${id}"]`).forEach(menu=>{menu.classList.toggle('hidden',!hasManagedData);const trigger=menu.querySelector('[data-source-menu-trigger]');if(trigger)trigger.disabled=busy;if(busy){trigger?.setAttribute('aria-expanded','false');menu.querySelector('.source-action-menu__popover')?.classList.add('hidden')}});document.querySelectorAll(`[data-delete="${id}"]`).forEach(button=>{button.disabled=busy;button.classList.toggle('hidden',!hasManagedData)});return result}
function resourceInstallationBusy(id){const status=resourceStates[id];return['running','validating','queued','pausing','cancelling'].includes(status?.download?.state)||['running','queued','pausing','cancelling'].includes(status?.prepare?.state)}
...
async function refreshDownloadStatus(providedSnapshot){
  if(refreshingResources)return resourceStates;
  refreshingResources=true;
  try{
    normalizeSourceCatalogControls();
    const ids=managedResourceIds();
    ids.forEach(ensureDeleteControl);
    const snapshot=providedSnapshot||await fetch('/api/resources/status').then(async response=>{
      const body=await response.json();
      if(!response.ok)throw new Error(body.error||'Data source status is not available');
      return body
    });
    const entries=ids.map(id=>[id,applyResourceStatus(id,snapshot.resources[id])]),states=Object.fromEntries(entries),setup=snapshot.setup,transcripts=states['ensembl-gff3'];
    entries.forEach(([id,state])=>applyWizardResourceState(id,state));
    const nextCoreId=!setup.referenceReady?'grch38-reference':!setup.transcriptCacheReady?'ensembl-gff3':null,nextCore=nextCoreId?states[nextCoreId]:null,coreBusy=Boolean(nextCore&&resourceInstallationBusy(nextCoreId));
    const coreLabel=setup.ready
      ?'Installed'
      :coreBusy
        ?`${nextCore.label} ${resourceTitle(nextCoreId)}`
        :!setup.referenceReady
          ?'GRCh38 reference not installed'
          :!setup.engineReady
            ?'Annotation engine needs repair'
            :transcripts?.label||'Ensembl transcript cache not installed';
    document.querySelectorAll('[data-reference-state]').forEach(node=>node.textContent=coreLabel);
    document.querySelector('.required-resource')?.classList.toggle('source-unavailable',!setup.ready);
    document.querySelectorAll('[data-update="ensembl-gff3"]').forEach(button=>button.classList.toggle('hidden',!setup.ready));
    document.querySelectorAll('[data-core-install]').forEach(coreButton=>{
      const idleAction=coreButton.dataset.coreInstallLabel||(nextCoreId==='ensembl-gff3'?'Install transcripts':'Install'),coreAction=coreBusy?`${nextCore.label}…`:idleAction;
      coreButton.classList.toggle('hidden',setup.ready||!nextCoreId);
      coreButton.disabled=coreBusy;
      coreButton.toggleAttribute('aria-busy',coreBusy);
      coreButton.innerHTML=`${prototypeIcon('download')}<span>${coreAction}</span>`
    });
    lastSetupReady=Boolean(setup.ready);
    document.querySelectorAll('[data-resource-review="core"]').forEach(row=>row.classList.toggle('ready',lastSetupReady));
    if(currentStep===4){
      const selectedReady=enabledSourceIds().every(id=>resourceStates[id]?.ready),ready=setup.ready&&selectedReady;
      $('#continue').disabled=!ready;
      $('#continue').innerHTML=ready?`${recoveryFiles?'Verify and recover':'Start annotation'} <span>→</span>`:'Install required annotation data'
    }
    updateWizardReadiness();
    updateReviewResourceStates();
    updateSetupModal(lastSetupReady);
    return states
  }finally{refreshingResources=false}
}
async function refreshAppStatus(){const response=await fetch('/api/status'),snapshot=await response.json();if(!response.ok)throw new Error(snapshot.error||'Application status is not available');await refreshDownloadStatus(snapshot.resources);await refreshAnnotationStatus(snapshot.annotation);await refreshTasks(snapshot.tasks);return snapshot}
async function deleteResource(id){const core=id==='grch38-reference',confirmed=await confirmDestructiveAction({title:`Remove ${resourceTitle(id)} data?`,message:`Downloaded parts and installed data for this source will be deleted.${core?' Local annotation will not be available until you install the shared core package again.':''}`,confirmLabel:'Remove data',cancelLabel:'Keep data'});if(!confirmed)return;const response=await fetch(`/api/resources/${id}/delete`,{method:'POST',headers:{'X-AnnoCat-CSRF':'1'}}),result=await response.json();if(!response.ok)showResourceNotice(result.error||'AnnoCAT did not remove the data source');await refreshAppStatus()}
function applyWizardResourceState(id,state){document.querySelectorAll(`#wizard-sources input[data-source="${id}"]`).forEach(input=>{input.disabled=!state.ready;input.title=state.ready?'':state.prepare.detail||'This source is not ready';if(!state.ready)input.checked=false;else if(selectedProfile()?.sourceIds.includes(id))input.checked=true;input.closest('.source-option')?.classList.toggle('source-unavailable',!state.ready)})}
...
async function requestResourceAction(id,path,update=false,rebuild=false){const query=path==='prepare/start'?`?concurrency=${installationConcurrency()}&sourceMode=${sourceInputMode()}${update?'&update=true':''}${rebuild?'&rebuild=true':''}`:'';const response=await fetch(`/api/resources/${id}/${path}${query}`,{method:'POST',headers:{'X-AnnoCat-CSRF':'1'}}),result=await response.json();if(!response.ok)throw new Error(result.error||`Could not ${path.replace('/',' ')}`);return result}
...
async function handleResourceCardInstall(button){
  ...
  const compatibility=resourceStates[id]?.prepare.state;
  if(compatibility==='rebuild-required'){
    const confirmed=await confirmDestructiveAction({title:`Rebuild ${resourceTitle(id)} cache?`,message:'Generated cache files will be replaced. Downloaded source data, retained source parts, and field settings will be kept.',confirmLabel:'Rebuild cache',cancelLabel:'Cancel'});
    if(!confirmed)return;
    button.disabled=true;
    try{await requestResourceAction(id,'prepare/start',false,true)}catch(error){showResourceNotice(error.message)}finally{button.disabled=false;await refreshAppStatus()}
    return
  }
...
async function startCoreInstall(button){
  if(button){
    button.disabled=true;
    button.setAttribute('aria-busy','true');
    button.innerHTML=`${prototypeIcon('download')}<span>Starting…</span>`
  }
  try{
    const snapshot=await fetch('/api/resources/status').then(r=>r.json()),failures=[];
    for(const id of['grch38-reference','ensembl-gff3']){
      const status=snapshot.resources[id],ready=status.prepare.state==='ready',busy=['running','validating','queued','pausing','cancelling'].includes(status.download.state)||['running','pausing','cancelling'].includes(status.prepare.state);
      if(ready||busy)continue;
      try{
        if(status.download.state==='downloaded')await requestResourceAction(id,'prepare/start');
        else await requestResourceAction(id,'download/start')
      }catch(error){failures.push(`${resourceTitle(id)}: ${error.message}`)}
    }
    if(failures.length)showResourceNotice(`Core annotation setup could not start: ${failures.join(' · ')}`)
  }finally{
    button?.removeAttribute('aria-busy');
    await refreshAppStatus()
  }
}
async function handleDownloadJobAction(id,action,button){if(action==='remove'){await deleteResource(id);return}if(action==='cancel'&&!await confirmDestructiveAction({title:`Cancel ${resourceTitle(id)} installation?`,message:'The task will stop and its downloaded and partially installed data will be discarded. Other installed data sources are not affected.',confirmLabel:'Cancel installation',cancelLabel:'Continue installation'}))return;button.disabled=true;if(['resume','install'].includes(action))button.classList.add('hidden');try{if(action==='pause')await requestResourceAction(id,'download/pause');else if(action==='cancel')await requestResourceAction(id,'download/cancel');else await toggleDownloadJob(id)}catch(error){showResourceNotice(error.message)}finally{button.disabled=false;await refreshAppStatus()}}
...
function installableProfileResources(profile){const requested=new Set(['grch38-reference','ensembl-gff3',...profileInstallSourceIds(profile)]);return resourcePlan.resources.filter(resource=>requested.has(resource.id)&&resource.state==='missing'&&!resourceStates[resource.id]?.ready)}
...
dbsnp:{id:['rsID','The rs identifier used to reference this variant in dbSNP.'],globalMaf:['Global minor-allele frequency','Legacy global minor-allele frequency carried by dbSNP.'],variantType:['Variant type','dbSNP type such as SNV, insertion, deletion, or microsatellite.'],common:['Common-variant flag','Whether dbSNP marks the allele as common in population data.']},
...
function profileReviewResources(profile,installable){const items=[...installable],seen=new Set(items.map(item=>item.id));for(const id of profileInstallSourceIds(profile)){if(seen.has(id)||!resourceStates[id]?.ready||!(id==='dbnsfp'||configurableSupplementarySources.has(id)))continue;const item=resourcePlan.resources.find(resource=>resource.id===id);if(item){items.push(item);seen.add(id)}}return items}
...
function updateProfileInstallRuntimeCopy(dialog){const count=installationConcurrency(),mode=sourceInputMode(),installable=Number(dialog.dataset.installableCount||0),total=Number(dialog.dataset.installNetworkBytes||0),unknownSizes=Number(dialog.dataset.installUnknownSizes||0),readyCache=Number(dialog.dataset.readyCacheBytes||0),download=unknownSizes?`${total?`${formatDataSize(total)} known download · `:''}${unknownSizes} size${unknownSizes===1?'':'s'} determined during installation`:`${formatDataSize(total)} download`,concurrency=count===1?'one data source at a time':`up to ${count} data sources at one time`;dialog.querySelector('[data-install-summary]').textContent=`${installable} data source${installable===1?'':'s'} · ${download} · ${concurrency}${readyCache?` · ${formatDataSize(readyCache)} installed`:''}`;dialog.querySelector('[data-install-stream-note]').textContent=mode==='pure-streaming'?'A direct download uses less disk. An interruption restarts the current source part.':'A resumable download keeps a temporary part. An interrupted download continues from this part.'}
...
async function showProfileInstallReview(profileId){
  ...
  if(!profile)return;
  const installable=installableProfileResources(profile),reviewItems=profileReviewResources(profile,installable),installableIds=new Set(installable.map(item=>item.id)),profileSources=profileInstallSourceIds(profile),pending=profileSources.filter(id=>id!=='fastvep'&&!installableIds.has(id)&&!resourceStates[id]?.ready).map(id=>sources.find(source=>source.id===id)?.name||id),total=installable.reduce((sum,item)=>sum+(item.downloadBytes||0),0),unknownSizes=installable.filter(item=>!item.downloadBytes).length,readyCache=profileSources.reduce((sum,id)=>sum+Number(resourceStates[id]?.prepare?.preparedBytes||0),0);
  ...
  pendingMessage.textContent=pending.length?`${pending.join(', ')} do not have verified installers and will not start.`:'';
...
async function start(){
  ...
  applyDefaultWizardProfile(savedProfile);
...
document.querySelectorAll('.nav-item').forEach(button=>button.addEventListener('click',()=>{showPage(button.dataset.page);if(button.dataset.page==='browse')refreshCompletedRuns().catch(console.error)}));$('#open-demo').addEventListener('click',async event=>{const button=event.currentTarget,label=button.querySelector('strong'),previous=label.textContent;button.disabled=true;button.setAttribute('aria-busy','true');label.textContent='Opening demo…';try{const response=await fetch('/api/demo'),body=await response.json();if(!response.ok)throw new Error(body.error||'The demo could not be opened');await openCompletedRun(body.run)}catch(error){$('#result-library-status').textContent=error.message}finally{button.disabled=false;button.removeAttribute('aria-busy');label.textContent=previous}});$('#back-to-browse').addEventListener('click',()=>{showPage('browse');refreshCompletedRuns().catch(console.error)});$('#choose-vcfs').addEventListener('click',()=>chooseVcfs().catch(error=>showResourceNotice(error.message)));$('#recover-annotation').addEventListener('click',()=>chooseRecoveryFiles().catch(error=>showResourceNotice(error.message)));$('#vcf-files').addEventListener('change',event=>{recoveryFiles=null;selectedVcfBuildConfirmed=false;selectedPaths=[...event.target.files].map(file=>file.name);renderSelectedPaths()});$('#profile').addEventListener('change',applyProfile);$('#browse-output').addEventListener('click',chooseFolder);$('#output-directory-fallback').addEventListener('change',event=>{const file=event.target.files[0];if(file){const folder=file.webkitRelativePath.split('/')[0];$('#output-folder').value=folder;$('#folder-message').textContent=`Selected “${folder}” using compatibility mode.`;setStep(3)}});$('#output-folder').addEventListener('input',()=>setStep(3));$('#continue').addEventListener('click',()=>{if(currentStep<4)setStep(currentStep+1);else startAnnotation()});$('#back-step').addEventListener('click',()=>setStep(currentStep-1));$('#search').addEventListener('input',()=>scheduleResultSearch());$('#search').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();scheduleResultSearch(true)}});$('#columns').addEventListener('click',event=>{event.stopPropagation();toggleResultPopover('column-menu')});start().catch(error=>console.error(error));
...
$('#setup-core-annotation').addEventListener('click',()=>{setupDismissed=true;$('#first-run').classList.add('hidden');applyDefaultWizardProfile('online');showPage('annotate');setStep(1)});$('#setup-offline').addEventListener('click',()=>{setupDismissed=true;$('#first-run').classList.add('hidden');showPage('resources')});$('#setup-open-results').addEventListener('click',openExistingResults);$('#setup-later').addEventListener('click',()=>{setupDismissed=true;$('#first-run').classList.add('hidden')});document.querySelector('#browse .choice:first-child').addEventListener('click',openExistingResults);
...
const resultScrollObserver=new IntersectionObserver(entries=>{if(entries.some(entry=>entry.isIntersecting))loadMoreResults()},{root:resultTableWrap,rootMargin:`${RESULT_PREFETCH_MARGIN_PX}px 0px`});resultScrollObserver.observe(resultScrollSentinel);
resultTableWrap.addEventListener('scroll',()=>{if(resultTableWrap.scrollHeight-resultTableWrap.scrollTop-resultTableWrap.clientHeight<RESULT_PREFETCH_MARGIN_PX)loadMoreResults()},{passive:true});
...
function closeResultPopovers(){resultPopovers.forEach(([panel,button])=>{$(`#${panel}`).classList.add('hidden');$(`#${button}`).setAttribute('aria-expanded','false')});phenotypeFeature.close()}
...
$('#result-library-search').addEventListener('input',event=>{completedRunSearch=event.target.value;renderCompletedRuns(completedRuns)});
document.addEventListener('click',event=>document.querySelectorAll('[data-result-menu][open]').forEach(menu=>{if(!menu.contains(event.target))menu.removeAttribute('open')}));
$('#rename-report').addEventListener('click',renameCurrentResult);
