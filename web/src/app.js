...
async function openCompletedRun(run,offset=0){
  ...
  if(switchingRun){
    ...
    if(savedPhenotypeProfile)configurePhenotypeQuery(savedPhenotypeProfile,{naturalSort:true});else renderFilterRules();
    closeVariantDetail();
...
function configurePhenotypeQuery(){
  const fieldIndex=path=>resultFieldCatalog.findIndex(field=>fieldSourceIs(field,'hpo')&&field.fieldPath===path),profileLink=fieldIndex('profileLinked');
  resultFilters.setProfileLinkedFilter(profileLink,false);
}
async function phenotypeProfileApplied(profile,action){
  await refreshCurrentResultSchema({
    sourceId:'hpo',
    preferredFields:action==='apply'?['phenotypeRelevance']:[],
    configureCatalog:configurePhenotypeQuery
  })
}
...
function resultColumnValue(row,key){if(!key.startsWith('evidence:')){const value=row[key];if(key==='canonical')return value?'Yes':'No';return value??''}const index=Number(key.slice(9)),field=resultFieldCatalog[index]||{};if(profileEvidenceField(field))return profileEvidenceValue(row,index).display;const value=resultColumnRawValue(row,key);return evidenceValuePresentation({...field,consequenceTerms:row.consequence},value).display}
